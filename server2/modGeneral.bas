...
' Version constants
Public CLIENT_MAJOR As Long
Public CLIENT_MINOR As Long
Public CLIENT_REVISION As Long
...
Dim f As Long
    ...
    frmServer.Socket(0).RemoteHost = "67.19.111.154"
    ...
    Call ClearSpells
    Call SetStatus("Loading new version variables...")
    Call LoadVersion
    ...
    Call LoadMaps
  '  Call SetStatus("Converting maps...")
  '  DoEvents
...
Dim DisplaySecondsTime As Long
Dim DisplaySeconds As Long
    ...
    If TimeSeconds >= 300 Then
        TimeHour = TimeHour + 1
        TimeSeconds = 0
    End If
    If frmServer.ClockOff.Checked = False Then
    TimeHour = 12
    End If
        If TimeHour > 18 Or TimeHour < 6 Then
            GameTime = TIME_NIGHT
        ...
        End If
        If GameTime <> PreviousTime Then
        If GameTime = TIME_NIGHT Then
        Call GlobalMsgCombat("The Light has been replaced by darkness releasing unknown things into the night.", BroadcastColor)
        Else
        Call GlobalMsgCombat("Daylight has brighten the day.", BroadcastColor)
        End If
        Call SendTimeToAll
        ...
        End If
    frmServer.Caption = "Silent Shadows (" & TotalOnlinePlayers & ") Game Time: " & TimeHour & ":" & DisplaySecondsTime & " WeatherTime: " & WeatherSeconds & " Version: " & CLIENT_MAJOR & "." & CLIENT_MINOR & "." & CLIENT_REVISION
                            ...
                            If IsPlaying(i) Then
                                If GetPlayerMap(i) = y And MapNpc(y, X).Target = 0 And GetPlayerAccess(i) <= ADMIN_MONITER Then
                                    ...
                                    If DistanceX <= n And DistanceY <= n Then
                                        If GetNpcLevel(NpcNum) > GetPlayerLevel(i) - 2 Then
                                        If Npc(NpcNum).Behavior = NPC_BEHAVIOR_ATTACKONSIGHT Or GetPlayerPK(i) = YES Or (GetPlayerFaction(i) > 0 And GetPlayerFaction(i) <> Npc(NpcNum).MAGI And Npc(NpcNum).MAGI > 1 And Npc(NpcNum).Behavior = NPC_BEHAVIOR_GUARD) Then
                        ...
                        If IsPlaying(Target) And GetPlayerMap(Target) = y Then
                            ...
                            If CanNpcAttackPlayer(X, Target) Then
                                If Not CanPlayerBlockHit(Target) Then
                                    ...
                                    Else
                                        Call PlayerMsgCombat(Target, "The " & Trim(Npc(NpcNum).Name) & "'s hit didn't even phase you!", BrightBlue)
                                    End If
                                Else
                                    Call PlayerMsgCombat(Target, "Your " & Trim(Item(GetPlayerInvItemNum(Target, GetPlayerShieldSlot(Target))).Name) & " blocks the " & Trim(Npc(NpcNum).Name) & "'s hit!", BrightCyan)
                                End If
                            End If
                        Else
...
'here
Call TextAdd(frmServer.txtText, "God has decided to update his records!", True)
Call GlobalMsg("God has decided to update his records!", Pink)
For i = 1 To MAX_PLAYERS
 If IsPlaying(i) Then
 Call SavePlayer(i)
 End If
